...
function formatTime(minutes, seconds) {
  const strTime = `${minutes.toString().padStart(2, 0)}:${seconds
    .toString()
    .padStart(2, 0)}`;
  return strTime;
}
...
function countVowels(str) {
  ...
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (arrVowels.includes(str.charAt(i).toLowerCase())) {
      count += 1;
    }
  }
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (
      ...
      strTemp += currentLetter.toLowerCase();
    }
  }
  ...
  for (let i = str.length - 1; i >= 0; i -= 1) {
    ...
    if (
      ...
      strPalindrome += currentLetter.toLowerCase();
    }
  }
  return strTemp === strPalindrome;
}
...
function findLongestWord(sentence) {
  if (sentence) {
    ...
    const returnWord = [arrWords[0], arrWords[0].length];

    for (let i = 1; i < arrWords.length; i += 1) {
      if (returnWord[1] < arrWords[i].length) {
        returnWord[0] = arrWords[i];
        returnWord[1] = arrWords[i].length;
      }
    }
